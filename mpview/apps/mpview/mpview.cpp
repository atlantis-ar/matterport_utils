...
// BAW
static bool render_seg_maps = false;
static bool didRedraw = false;
std::mutex redraw_flag_mutex;
...
PrintUsage(void)
{
  ...
  // BAW
  printf("  -seg_maps : render class and instance segmentations maps (output_image is interpreted as directory to store images\n");
  //
  printf("\n");
...
void GLUTRedraw(void)
{
	...
	std::unique_lock<std::mutex> lock(redraw_flag_mutex);
  ...
  // BAW
  if (render_seg_maps) {
	  region_draw_flags.Remove( MP_DRAW_BBOXES );
	  object_draw_flags.Remove(MP_DRAW_BBOXES);
	  image_draw_flags.Remove(MP_DRAW_BBOXES);

	  region_draw_flags.Add(MP_DRAW_FACES);
	  object_draw_flags.Add(MP_DRAW_FACES);
	  scene_draw_flags.Add(MP_DRAW_FACES);
...
ParseArgs(int argc, char **argv)
{
  ...
  while (argc > 0) {
    if ((*argv)[0] == '-') {
	  ...
	  // BAW
	  else if (!strcmp(*argv, "-seg_maps")) render_seg_maps = true;
	  ///
      else {
...
void processMaps() {
	...
	std::string out_image_base_name = output_image_filename;

	snap_image_index = 0;
	while (snap_image_index < house->images.NEntries()) {
		SnapImage(house->images.Kth(snap_image_index));

		char camId[20];
		char yawId[20];
		sprintf_s(camId, "%d", house->images.Kth(snap_image_index)->camera_index);
		sprintf_s(yawId, "%d", house->images.Kth(snap_image_index)->yaw_index);

		std::string full_output_image_name = out_image_base_name + "_classes/" + std::string(house->images.Kth(snap_image_index)->name) + "_c" + camId + "_" + yawId + ".png";

		printf("writing %s \n", full_output_image_name.c_str());

		output_image_filename = (char*)full_output_image_name.c_str();
		...
		full_output_image_name = out_image_base_name + "_instances/" + std::string(house->images.Kth(snap_image_index)->name) + "_i" + camId + "_" + yawId + ".png";
