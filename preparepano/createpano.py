...
def getAngles(matrixDict):
    v = np.zeros((18,2))
    
    euler = np.array([ Rot.from_matrix(inv(np.transpose(matrixDict[0][0]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[0][1]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[0][2]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[0][3]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[0][4]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[0][5]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[1][0]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[1][1]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[1][2]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[1][3]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[1][4]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[1][5]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[2][0]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[2][1]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[2][2]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[2][3]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[2][4]))[0:3,0:3]).as_euler('xyz'),
              Rot.from_matrix(inv(np.transpose(matrixDict[2][5]))[0:3,0:3]).as_euler('xyz') ])
...
def combineViews(images,v,outsize,blending=True,depth=False):
    ...
    for i in range(len(images)):
        ...
        (sphereImg,validMap) = im2Sphere(images[i][imcutout[0][0]:imcutout[0][1],imcutout[1][0]:imcutout[1][1]]
                   ,default_fov,outsize[0],outsize[1],v[i,0],v[i,1],blending,i,depth)
  
        sphereImg[validMap<0.00000001] = 0
        
        if blending:
            pano = pano + sphereImg
        else:
            if depth:
                sphereImg[:,:,0] = sphereImg[:,:,0] * validMap
                pano = pano + sphereImg 
            else:
...
def im2Sphere(im,imHoriFOV,sphereW,sphereH,x,y,interpolate,nr,weightByCenterDist=False):
    ...
    TX = TX.flatten('F')
    TY = TY.flatten('F')
    ANGx = ((TX- (sphereW/2) -0.5)/sphereW) * math.pi *2 
    ...
    # warp image
    sphereImg = warpImageFast(im, Px, Py,interpolate,(sphereW,sphereH),nr)

    validMap = np.zeros((sphereImg.shape[0],sphereImg.shape[1]))
    validMap[:,:] = np.logical_not(np.isnan(sphereImg[:,:,0])).astype(float)
    ...
    if weightByCenterDist:
        ...
        weightImWarped = warpImageFast(weightIm, Px, Py,False,(sphereW,sphereH),nr)

        validMap = weightImWarped[:,:,0]
        validMap[sphereImg[:,:,0]<1] = 0

    else:
        validMap[sphereImg[:,:,0]<0] = 0
...
def warpImageFast(im,XXdense,YYdense,interpolate,outsize,nr):
